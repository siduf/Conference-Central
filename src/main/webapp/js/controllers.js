...
conferenceApp.controllers.controller('MyProfileCtrl',
    function ($scope, $log, oauth2Provider, HTTP_ERRORS) {
        ...
        $scope.isValidZipCode = function () {
            if (!$scope.profile.zipcode || $scope.profile.zipcode.length == 0) {
                return true;
            }
            return /^[\d]+$/.test($scope.profile.zipcode) && $scope.profile.zipcode >= 0;
        }
 ...
 */
conferenceApp.controllers.controller('CreateFieldCtrl',
    function ($scope, $log, oauth2Provider, HTTP_ERRORS) {
        ...
        /**
         * Holds the default values for the input candidates for crop types select.
         * @type {string[]}
         */
        $scope.crops = [
            'Rice',
         ...
         */
        $scope.isfieldZipCode = function () {
            if (!$scope.field.fieldZipCode || $scope.field.fieldZipCode.length == 0) {
                return true;
            }
            return /^[\d]+$/.test($scope.field.fieldZipCode) && $scope.field.fieldZipCode >= 0;
        }
         ...
         */
        $scope.isValidField = function (fieldForm) {
            return !fieldForm.$invalid &&
                $scope.isfieldZipCode();
        }
        ...
        $scope.createField = function () {
            if (!$scope.isValidField(fieldForm)) {
                return;
            }
            ...
            gapi.client.conference.createField($scope.field).
                execute(function (resp) {
                    $scope.$apply(function () {
                        $scope.loading = false;
                        if (resp.error) {
                            // The request has failed.
                            var errorMessage = resp.error.message || '';
                            $scope.messages = 'Failed to create a Field : ' + errorMessage;
                            $scope.alertStatus = 'warning';
                            $log.error($scope.messages + ' Field : ' + JSON.stringify($scope.field));
                            ...
                            // The request has succeeded.
                            $scope.messages = 'The Field Profile has been created : ' + resp.result.name;
                            $scope.alertStatus = 'success';
                            $scope.submitted = false;
                            $scope.field = {};
                            $log.info($scope.messages + ' : ' + JSON.stringify(resp.result));
